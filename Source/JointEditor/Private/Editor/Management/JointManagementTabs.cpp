...
TSharedRef<SWidget> SJointEditorUtilityTab::CreateInvalidateSection()
{
	return SNew(SJointOutlineBorder)
		...
		.NormalColor(FLinearColor(0.015f, 0.015f, 0.02f))
		.HoverColor(FLinearColor(0.04f, 0.04f, 0.06f))
		.OutlineNormalColor(FLinearColor(0.015f, 0.015f, 0.02f))
		.OutlineHoverColor(FLinearColor(0.5f, 0.5f, 0.5f))
		...
		[
			...
			[
				...
				[
					...
					.Text(LOCTEXT("CleanUpNodes",
					              "Clean-up orphened nodes in the projects. This doesn't need to be done twice, it's for the assets from old versions (before 2.8f)"))
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.HAlign(HAlign_Right)
			...
			[
				...
				.Text(LOCTEXT("2.9Tooltip", "Joint 2.9f"))
			]
			+ SVerticalBox::Slot()
			.AutoHeight()
			.Padding(FJointEditorStyle::Margin_Normal)
			[
				...
				[
					...
					.Text(LOCTEXT("UpdateEdSettingsNodes",
					              "Move the old editor node setting related properties to the new structure. This doesn't need to be done twice, it's for the assets from old versions. (before 2.9f)"))
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.HAlign(HAlign_Right)
...
void SJointEditorNodeClassManagementTab::Construct(const FArguments& InArgs)
{
	...
	TSharedPtr<FTabManager::FLayout> DebuggerLayout = FTabManager::NewLayout("JointNodeClassManagementTab_V1.1f")
...
void FJointEditorTap_MissingClassInstance::Construct(const FArguments& InArgs)
{
	...
	ChildSlot[
		...
		.OutlineNormalColor(FLinearColor(0.015f, 0.015f, 0.02f))
		.OutlineHoverColor(FLinearColor(0.5f, 0.5f, 0.5f))
		.ContentPadding(FJointEditorStyle::Margin_Normal * 2.f)
		.HAlign(HAlign_Fill)
		.VAlign(VAlign_Fill)
		[
			SNew(SHorizontalBox)
